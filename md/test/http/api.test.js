...
test("inscricao publica e listada, consultada em detalhe e atualizada no painel administrativo", async (context) => {
  ...
  assert.equal(listData[0].protocol, submitData.protocol);
  assert.equal(listData[0].status, "new");
  assert.equal(listData[0].teamName, "Equipe Integracao");
  assert.equal(listData[0].responsibleEmail, "tecnico@example.test");
